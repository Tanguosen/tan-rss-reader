...
async def my_subscriptions(current=Depends(get_current_user), session: AsyncSession = Depends(get_session)) -> List[Channel]:
    ...
    fallback_icons = {}
    channels_missing_icons = [c.id for c in channels if not c.icon_url]
    if channels_missing_icons:
        from ..models import ChannelSource as SAChannelSource, Feed as SAFeed
        ...
        if translation_json:
            try:
                ...
                if "title" in t_data and isinstance(t_data["title"], dict):
                    # just pick the first available translation or specifically zh-CN
