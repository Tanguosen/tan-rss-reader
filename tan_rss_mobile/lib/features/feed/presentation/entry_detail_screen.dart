...
class _EntryDetailContentState extends ConsumerState<_EntryDetailContent> {
  ...
  Future<void> _trackView() async {
    ...
    await repository.trackEntryView(_entry.id);
    if (!mounted) return;
  ...
  Future<void> _generateSummary() async {
    ...
    final repository = ref.read(feedRepositoryProvider);
    try {
      final result = await repository.summarizeEntry(
        entryId: _entry.id,
        language: 'zh',
      );
      if (!mounted) return;
  ...
  Future<void> _translateContent() async {
    ...
    final repository = ref.read(feedRepositoryProvider);
    try {
      final content = await repository.translateContent(
        entryId: _entry.id,
        language: 'zh',
      );
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final content =
        (_useTranslatedContent && (_translatedContent?.isNotEmpty ?? false))
        ? _translatedContent!
        : rawContent;
    final displayTitle =
          ...
          children: [
                  ...
                  Row(
                    children: [
                      IconButton(
                        onPressed: _saving ? null : _toggleStar,
                        icon: Icon(
                          _entry.starred ? Icons.star : Icons.star_border,
                        ),
                      ),
                      IconButton(
                        onPressed: _openOriginal,
            ...
            Wrap(
              ...
              children: [
                ...
                FilledButton.tonalIcon(
                  onPressed: _loadingContentTranslation
                      ? null
                      : _translateContent,
